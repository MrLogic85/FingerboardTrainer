...
public class MainActivity extends Activity {
	...
	private void start() {
		...
		if (mBinder != null) {
			...
			mRunning = true;
			mBinder.getService().startTimer(mHangTime, mPauseTime, mRepetitions, mRestTime, mTotalRepetitions,
					meNotification);
	...
	public void onTrainingCompleted(final int hangTime, final int pauseTime, final int repetitions, final int restTime,
		...
		mHandler.post(new Runnable() {
			public void run() {
				...
				log += " - Hang/Pause " + hangTime + "s/" + pauseTime + "s, Repetitions " + repetitions + ", Rest "
						+ (restTime / 60) + "m " + (restTime % 60) + "s, Total repetitions " + totalRepetitions;
				history.add(log);
