...
public class BillingManager {
    ...
    private boolean checkInAppBillingSupport() {
        if (mBillingService != null) {
            try {
                int billingSupport = mBillingService.isBillingSupported(3,
                        mActivity.getPackageName(), "inapp");
    ...
    private void getPurchasedItems() {
        try {
            Bundle ownedItems = mBillingService.getPurchases(3, mActivity.getPackageName(),
                    "inapp", null);
